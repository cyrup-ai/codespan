...
impl<'writer, 'config> Renderer<'writer, 'config> {
    ...
    pub fn render_snippet_source(
        ...
        // ```
        {
            ...
            // Extract style before the loop to avoid borrow conflicts
            #[cfg(feature = "termcolor")]
            let primary_style = self.styles().label(severity, LabelStyle::Primary);

            let char_metrics: std::vec::Vec<_> = self.char_metrics(source.char_indices()).collect();
            for (metrics, ch) in char_metrics {
                ...
                match ch {
                    '\t' => {
                        let width = metrics.unicode_width;
                        for _ in 0..width {
                            write!(self, " ")?;
                        }
                    }
        ...
        if !single_labels.is_empty() {
            ...
            // Extract chars and styles before the loop to avoid borrow conflicts
            let chars = self.chars();
            let styles = self.styles();
            let single_primary_caret = chars.single_primary_caret;
            let single_secondary_caret = chars.single_secondary_caret;

            let char_metrics: std::vec::Vec<_> = self
                ...
                if previous_label_style != current_label_style {
                    match current_label_style {
                        ...
                        Some(label_style) => {
                            #[cfg(feature = "termcolor")]
                            {
                                let style = styles.label(severity, label_style);
    ...
    fn caret_pointers(
        ...
        // Extract references before the loop to avoid borrow conflicts
        let styles = self.styles();
        let chars = self.chars();
        let pointer_left = chars.pointer_left;

        let char_metrics: std::vec::Vec<_> = self.char_metrics(char_indices).collect();
        for (metrics, ch) in char_metrics {
            let column_range = metrics.byte_index..(metrics.byte_index + ch.len_utf8());
            ...
            if metrics.byte_index <= max_label_start {
                let spaces_range: std::vec::Vec<_> = spaces.collect();
                for _ in spaces_range {
                    write!(self, " ")?;
                }
            }
        }
    ...
    fn label_multi_top_caret(
        ...
        // Extract character before the loop to avoid borrow conflicts
        let multi_top_char = self.chars().multi_top;

        let char_metrics: std::vec::Vec<_> = self
            .char_metrics(source.char_indices())
            .take_while(|(metrics, _)| metrics.byte_index < start + 1)
            .collect();
        for (metrics, _) in char_metrics {
            // FIXME: improve rendering of carets between character boundaries
            let width = metrics.unicode_width;
            for _ in 0..width {
                write!(self, "{}", multi_top_char)?;
            }
        }
    ...
    fn label_multi_bottom_caret(
        ...
        // Extract character before the loop to avoid borrow conflicts
        let multi_bottom_char = self.chars().multi_bottom;
...
impl WriteColor for Renderer<'_, '_> {
    ...
    fn set_hyperlink(&mut self, link: &termcolor::HyperlinkSpec) -> io::Result<()> {
